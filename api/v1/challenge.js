/**
 * POST /api/v1/challenge — contradiction argumentée d'une thèse.
 *
 * Corps :
 *   { these: string, persona?: 'architect'|'opponent'|'arbiter'|'strategist',
 *     friction?: 'doux'|'moyen'|'extreme', contexte?: string }
 *
 * Réponse :
 *   { reponse: string, persona, friction, credits_restants, usage }
 *
 * Coût : 1 crédit.
 */
...
export default async function handler(req, res) {
  ...
  const { these, persona = 'opponent', friction = 'moyen', contexte } = req.body || {};

  if (typeof these !== 'string' || these.trim().length < 3) {
    return res.status(400).json({
      erreur: 'parametre_invalide',
      message: 'Le champ « these » est requis (chaîne de 3 caractères minimum).',
    });
  }
  if (these.length > 4000) {
    return res.status(400).json({ erreur: 'parametre_invalide', message: '« these » dépasse 4000 caractères.' });
  }
  if (!PERSONAS[persona]) {
    return res.status(400).json({
      erreur: 'parametre_invalide',
      message: `« persona » doit valoir : ${Object.keys(PERSONAS).join(', ')}.`,
    });
  }
  if (!FRICTIONS[friction]) {
    return res.status(400).json({
      erreur: 'parametre_invalide',
      message: `« friction » doit valoir : ${Object.keys(FRICTIONS).join(', ')}.`,
    });
  }

  const auth = await autoriser(req, 'challenge');
  if (!auth.ok) {
    entetesApi(res);
